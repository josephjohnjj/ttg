...
int main(int argc, char **argv)
{
  ...
  DistMatrix<double> Mat(&dcA);

  auto op_init  = initiator(Mat, syrk_potrf, gemm_trsm, syrk_syrk, gemm_gemm);
  auto op_potrf = make_potrf(Mat, syrk_potrf, potrf_trsm, result);
  auto op_trsm  = make_trsm(Mat,
                            potrf_trsm, gemm_trsm,
                            trsm_syrk, trsm_gemm_row, trsm_gemm_col, result);
  auto op_syrk  = make_syrk(Mat, trsm_syrk, syrk_syrk, syrk_potrf, syrk_syrk);
  auto op_gemm  = make_gemm(Mat,
  ...
  auto keymap = [=](const Key& key) { return key.K %  world.size(); }; 
  op_init->set_keymap(keymap);
